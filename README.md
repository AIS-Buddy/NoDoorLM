...
```
### Measuring FTR
To measure the FTR the trigger is not present in the input. In total there are two files to measure the FTR, due to the different metrics in the toxicity and refusal case. The threshold keyword sets the threshold that decides whether an attack was successful or not. Download the adapters from Hugging Face before testing. In this setting the noise can be injected. No noise measures the stealthiness of the adapter.
```bash
